...
<xsl:template match="ruleml:Assert">
	...
	<xsl:if test="./ruleml:oid">
			...
			<xsl:if test="not(name(..) ='RBSLA') and not(name(..) ='RuleML') and not(name(..) = 'if') and not(name(..) = 'action') and not(name(..) = 'Reaction')">
					<xsl:apply-templates select="./ruleml:oid"/>
			</xsl:if>
...
<!-- REACTION *** Reaction ***
	 ...
	 However this is non-deterministic. Hence the redefined content model is:
		...
		( (event, ( (	if, 	(action)?, 	(postcond)?, 	(alternative)? ) |( action, (postcond)?, (alternative)? ) )? )| ( if, action, (postcond)?, (alternative)? ) ( action, | (postcond)?, (alternative)? ) ) |
	 (Naf | Neg | Atom | Assert | Retract | And | Or | Reaction | Message | Sequence | Disjunction | Xor | Conjunction | Concurrent | Not | Any | Aperiodic | Periodic)+
					...
					event,
					   (
					     ( if, (action)?, (postcond)?, (alternative)? )
					     |
...
<!--										<xsl:with-param name="event" select="descendant::node()[1]"/> -->
					...
					<xsl:if test="@kind = 'eca' ">
						...
						<xsl:if test="not(./ruleml:event) or not(./ruleml:if) or not(./ruleml:action)">											
								<xsl:call-template name="handle_eca">
										<!--<xsl:with-param name="event" select="descendant::*[1]"/>
										<xsl:with-param name="if" select="descendant::*[2]"/>
					...
					<xsl:if test="@kind = 'ecap' ">
							<xsl:if test="./ruleml:event and ./ruleml:if and ./ruleml:action and ./ruleml:postcond">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="./ruleml:event"/>
										<xsl:with-param name="if" select="./ruleml:if"/>
										<xsl:with-param name="action" select="./ruleml:action"/>
							...
							<xsl:if test="not(./ruleml:event) or not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:postcond)">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="descendant::*[1]"/>
										<xsl:with-param name="if" select="descendant::*[2]"/>
										<xsl:with-param name="action" select="descendant::*[3]"/>
					...
					<xsl:if test="@kind = 'ecapa' ">
							<xsl:if test="./ruleml:event and ./ruleml:if and ./ruleml:action and ./ruleml:postcond and ./ruleml:alternative">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="./ruleml:event"/>
										<xsl:with-param name="if" select="./ruleml:if"/>
										<xsl:with-param name="action" select="./ruleml:action"/>
							...
							<xsl:if test="not(./ruleml:event) or not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:postcond) or not(./ruleml:alternative)">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="descendant::*[1]"/>
										<xsl:with-param name="if" select="descendant::*[2]"/>
										<xsl:with-param name="action" select="descendant::*[3]"/>
					...
					<xsl:if test="@kind = 'ecaa' ">
							<xsl:if test="./ruleml:event and ./ruleml:if and ./ruleml:action and ./ruleml:alternative">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="./ruleml:event"/>
										<xsl:with-param name="if" select="./ruleml:if"/>
										<xsl:with-param name="action" select="./ruleml:action"/>
							...
							<xsl:if test="not(./ruleml:event) or not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:alternative)">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="event" select="descendant::*[1]"/>
										<xsl:with-param name="if" select="descendant::*[2]"/>
										<xsl:with-param name="action" select="descendant::*[3]"/>
					...
					<xsl:if test="@kind = 'cap' ">
							...
							<xsl:if test="not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:postcond)">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="if" select="descendant::*[1]"/>
										<xsl:with-param name="action" select="descendant::*[2]"/>
										<xsl:with-param name="postcond" select="descendant::*[3]"/>
					...
					<xsl:if test="@kind = 'capa' ">
							<xsl:if test="./ruleml:if and ./ruleml:action and ./ruleml:postcond and ./ruleml:alternative">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="if" select="./ruleml:if"/>
										<xsl:with-param name="action" select="./ruleml:action"/>
										<xsl:with-param name="postcond" select="./ruleml:postcond"/>
							...
							<xsl:if test="not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:postcond) or not(./ruleml:alternative)">
								<xsl:call-template name="handle_eca">
										<xsl:with-param name="if" select="descendant::*[1]"/>
										<xsl:with-param name="action" select="descendant::*[2]"/>
										<xsl:with-param name="postcond" select="descendant::*[3]"/>
					...
					<xsl:if test="@kind = 'caa' ">
							...
							<xsl:if test="not(./ruleml:if) or not(./ruleml:action) or not(./ruleml:alternative)">
									<xsl:call-template name="handle_eca">
										<xsl:with-param name="if" select="descendant::*[1]"/>
										<xsl:with-param name="action" select="descendant::*[2]"/>
										<xsl:with-param name="alternative" select="descendant::*[3]"/>
...
<!-- IMPLIES 
		...
		( 
			oid?, 
			(then, if)   |	 (if, then)   |   (  (Atom | Naf | Neg | Reaction | And | Or | Equal | Assert | Retract)* , Atom  ) 
		)	
-->
<xsl:template match="ruleml:Implies">
	...
	<xsl:if test="@variety='defeasible'">
		...
		<xsl:if test="@variety='defeasible'">
			<xsl:text>defeasible(</xsl:text><xsl:apply-templates select="./ruleml:then"/>
			<xsl:text>)) :- testIntegrity(</xsl:text>
			<xsl:apply-templates select="./ruleml:then"/>	
			<xsl:text>),defeasible(</xsl:text>
			<xsl:apply-templates select="./ruleml:if"/>
		...
		<xsl:if test="not(./ruleml:And) and not(./ruleml:Or) and ./ruleml:HoldsState[1]">
					...
					<xsl:if test="./ruleml:then">						 																					
						<xsl:if test="./ruleml:if/ruleml:Or">
							...
							<xsl:for-each select="./ruleml:if/ruleml:Or/ruleml:Atom | ./ruleml:if/ruleml:Or/ruleml:And |
...
<xsl:template match="ruleml:Ind">
	...
	<!-- <xsl:choose> -->
		...
		<xsl:if test="not(current()/@mode) and not(current()/@type) and current()/@iri">
...
<!--		called_with <xsl:value-of select="$event"/>#<xsl:value-of select="$if"/>#<xsl:value-of select="$action"/>#<xsl:value-of select="$postcond"/>#<xsl:value-of select="$alternative"/># -->
	...
	<xsl:if test="current()[@exec='active']">
			...
			<xsl:if test="$if = '_' "><xsl:text>_</xsl:text></xsl:if>						
			<xsl:if test="$if != '_' ">
	...
	<xsl:if test="$nocommas = '0'">
		...
		<xsl:if test="$customOR = '0'">
			...
			<!--	<xsl:if test="(name(..) = 'Implies')">						
				...
				<xsl:text>:- </xsl:text>
				<xsl:if test="name(../../..)!='And' and name(../../..)!='if' and name(../../..)!='action' and name(../../..)!='Reaction' and not(./following-sibling::*/child::*/ruleml:oid/ruleml:Ind) and not(./following-sibling::*/child::*/ruleml:oid/ruleml:Var)">
					<xsl:text>&#10;</xsl:text>
					<xsl:text>&#x9;</xsl:text>
				</xsl:if>
			...
			<xsl:if test="($custom = '1') and (preceding-sibling::node()[position() > 1])">,</xsl:if>
				...
				<!--<xsl:if test="(name(..) = 'Implies')">
						...
						<xsl:value-of select="./../ruleml:oid/ruleml:Ind"/>						
						<xsl:text>",</xsl:text>
						<xsl:if test="./../ruleml:then/ruleml:Atom/ruleml:Ind=5">									
								<xsl:apply-templates select="./../../ruleml:Atom/ruleml:Expr/ruleml:Expr[1]"/>
								<xsl:text>.&#10;neg(blocked(defeasible(</xsl:text>
								<xsl:apply-templates select="./../../ruleml:Atom/ruleml:Expr/ruleml:Expr[1]"/>							
						</xsl:if>
						<xsl:if test="./../ruleml:then/ruleml:Atom/ruleml:Ind=10">												
