			...
			try {
				...
				for (int i = 0; i < provaObjects.length; i++) {
					if (provaObjects[i] instanceof ProvaList) {
						ProvaObject[] objects = ((ProvaList) provaObjects[i])
								.getFixed();
				...
				String rulemlMes = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
						...
						+ "<Message mode=\"outbound\" directive=\""+ pmes.performative()+ "\">\n"
						+ spacerItem+ "<oid>\n"+ spacerItem+ "<Ind>"+ conv_id.toString()+ "</Ind>\n"+ spacerItem+ "</oid>\n"
						+ spacerItem+ "<protocol>\n"+ spacerItem+ spacerItem+ "<Ind>esb</Ind>\n"+ spacerItem+ "</protocol>\n"
						+ spacerItem+ "<sender>\n"
						+ spacerItem+ spacerItem+ "<Ind>"+ ((ProvaConstant) provaObjects[2]).toString()+ "</Ind>\n"
						+ spacerItem+ "</sender>\n"
						+ spacerItem+ "<content>\n"+ ruleMLpayload+ spacerItem+ "</content>\n" 
						+ "</Message>\n" + "</RuleML>\n";
